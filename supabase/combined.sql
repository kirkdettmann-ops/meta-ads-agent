...
--   0019  tenant_brand → multi-brand (1:N) + get_tenant_brands + slug-based lookup
--
...
--          hold one brand identity per tenant â€” for Hops + Perks we need
...
--     ordered by sort_order â€” used by the brand switcher in the header.
...
-- watermark_svg becomes optional â€” the new hero uses logo_url instead.
...
-- exist yet) â€” seed-tenant.ts handles brand creation in that case.
-- ============================================================================
...
begin
  ...
  if v_demo_tenant_id is not null then
    ...
    raise notice 'No Comedy Club Co brand row found â€” skipping demo data fix-up (fresh DB, seed-tenant.ts handles it).';
  end if;
end;
$$;
...
--    rows at all â€” so the UI never breaks for a fresh tenant.
-- ============================================================================
...
comment on function public.get_tenant_brand(uuid, text) is
  'Returns one brand for the tenant. p_slug NULL â†’ primary brand; p_slug
   specified â†’ that brand. Falls back to hardcoded Hops defaults if the
...
-- ============================================================================
-- 9. RPC: upsert_tenant_brand(...) â€” gains slug, kind, is_active,
