...
import { AlertCircle, CheckCircle2, ListChecks, TrendingUp, Video } from "lucide-react";
import Link from "next/link";

export const dynamic = "force-dynamic";

export default async function MetaVideoDashboardPage() {
  const { profile } = await requireUserWithProfile();
  const supabase = await createClient();
  const tenantId = profile.tenant_id;

  // Same RPCs as the image view — the two views are filtered views of the
  // same Meta data. When live data flows, the filter is on creative_type.
  ...
  return (
    <div className="space-y-6">
      ...
      <DashboardHero
        ...
        badge="Reels + in-stream"
        badgeVariant="secondary"
        date={new Date()}
        subtitle="Video spend, recommendations, and view-through metrics across your Meta video campaigns — Reels, in-stream, and Stories."
      />
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Video spend today</CardTitle>
